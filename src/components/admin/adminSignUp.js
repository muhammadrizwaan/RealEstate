...
import { View, Text, ImageBackground, Image, TouchableOpacity, KeyboardAvoidingView, AsyncStorage } from 'react-native';
...
async onSignin(email, password){
    ...
    this.setState({errorText:'', loading:true, userId:''});
    ...
    if(this.state.email==='' || this.state.password === '')
    {
     ...
     await axios.post(`http://www.tohfayhut.com/realestate/public/api/adminlogin`,
      {
        "email": email,
        "password": password
      }
    ...
    return (
      <KeyboardAvoidingView style={{flex:1, alignItems:'center', justifyContent:'center'}} behavior="padding">
        ...
        <Icon
            ...
            size={45}
          />
          <Text style={{fontSize:17, color:'#E61E26', marginVertical:height(1) }}>Admin</Text>
        </View>
          <Input
            autoCorrect={false}
            autoCapitalize="none"
          ...
          <Text style={{color:'red', fontSize: 20, alignSelf:'center', marginTop:15}}>{this.state.errorText}</Text>   
                ...
                {/* <TouchableOpacity style={{flexDirection:'row',marginVertical:'3%',height:height(5)}} onPress={()=>{this.props.navigation.navigate('signUp')}}>
                  <Text style={{color:'#1B1464', fontSize:14}}>Don't have Account?</Text>
                  <Text style={{color:'#1B1464',fontWeight:'bold', fontSize:16}}> Register</Text>
                  </TouchableOpacity> */}
        </View>
        </ImageBackground>
