...
class homeInspectorSignIn extends Component {
  constructor(props) {
    super(props);
    this.state= {
      email: '',
 ...
 onSignin(email, password){
    ...
    if(this.state.email==='' || this.state.password === '')
    {
      ...
      axios.post(`http://www.tohfayhut.com/realestate/public/api/inspectorlogin`,
      {
        "email": email,
        "password": password
      }
    ...
    return (
      <KeyboardAvoidingView style={{flex:1, alignItems:'center', justifyContent:'center'}} behavior="padding">
        ...
        <Icon
            ...
            size={45}
          />
          <Text style={{fontSize:17, color:'#E61E26', marginVertical:height(1) }}>Home Inspector</Text>
        </View>
          <Input
            autoCorrect={false}
            autoCapitalize="none"
          ...
          <Text style={{color:'red', fontSize: 20, alignSelf:'center', marginTop:15}}>{this.state.errorText}</Text>
                ...
                <TouchableOpacity style={{flexDirection:'row',marginVertical:'3%',height:height(5)}} onPress={()=>{this.props.navigation.navigate('homeInspectorSignUp')}}>
...
export default homeInspectorSignIn;
